...
;CONSTRUCTORES--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;OPCION (#2)
;Dom: code (Int)  X message (String)  X ChatbotCodeLink (Int) X InitialFlowCodeLink (Int) X Keyword (lista string)
;Rec: Opcion
(define option
...
;FLUJO (#3)
;Dom: id (int) X name-msg (String)  X Option (lista de opciones)
;Rec: Flujo
(define flow
...
;CHATBOT (#5)
;Dom: chatbotID (int) X name (String) X welcomeMessage (String) X startFlowId(int) X  flows (Lista de flujos)
;Rec: Opcion
(define chatbot
...
;Dom: name (string) X InitialChatbotCodeLink (Int) X chatbots (lista de chatbots)
;Rec: Sysytem
(define system
...
;GET------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
(define get-flow-id (lambda (flow) (car flow)))                                                   ;Dom:Fujo  /  Rec:Id (Integer)
(define get-flow-name (lambda (flow) (car (cdr flow))))                                           ;Dom:Fujo  /  Rec:name (String)
(define get-flow-opciones (lambda (flow) (car (cdr (cdr flow)))))                                 ;Dom:Fujo  /  Rec:name (String)

(define get-chatbot-id (lambda (chatbot) (car chatbot)))                                          ;Dom:Chatbot  /  Rec:Id (Integer)
(define get-chatbot-name (lambda (chatbot) (car (cdr chatbot))))                                  ;Dom:Chatbot  /  Rec:name (String)
(define get-chatbot-welcomeMessage (lambda (chatbot) (car (cdr (cdr chatbot)))))                  ;Dom:Chatbot /  Rec:welcomeMessage (String)
(define get-chatbot-startFlowId (lambda (chatbot) (car (cdr (cdr (cdr chatbot))))))               ;Dom:Chatbot  /  Rec:startFlowId (Integer)
(define get-chatbot-flows (lambda (chatbot) (car (cdr (cdr (cdr (cdr chatbot)))))))               ;Dom:Chatbot  /  Rec:flujos (Lista de flujos)

(define get-system-name (lambda (system) (car system)))                                           ;Dom:System  /  Rec:Name (String)
(define get-system-InitialChatbot (lambda (system) (car (cdr system))))                           ;Dom:System  /  Rec:InitialChatbot (Integer)
(define get-system-Users (lambda (system) (car (cdr (cdr system)))))                              ;Dom:System  /  Rec:Users (lista de string)
(define get-system-CurrentUser (lambda (system) (car (cdr (cdr (cdr system))))))                  ;Dom:System  /  Rec:Current user (String)
(define get-system-ChatHistory (lambda (system) (car (cdr (cdr (cdr (cdr system)))))))            ;Dom:System  /  Rec:ChatHistory (Lista de Histories)
(define get-system-chatbots (lambda (system) (car (cdr (cdr (cdr (cdr (cdr system))))))))         ;Dom:System  /  Rec:Chatbots (Lista de Chatbots)
...
(define system-login
  (lambda (system user)
    (if (and (member user (get-system-Users system)) (equal? "" (get-system-CurrentUser system)))
...
(define system-logout
  (lambda (system)
    (list (get-system-name system) (get-system-InitialChatbot system) (get-system-Users system) "" (get-system-ChatHistory system) (get-system-chatbots system))))
...
;Dom: Usuario x Lista 
;Rec: History (elemeto de la lista)
...
;Dom: Sistema x Opcion (String or Integer)
;Rec: Sistema
...
;Cuenta las veces que el usuario "user" ha interactuado con el sistema
;Dom: Usuario (String) x LstCH (Lista de chatbot)
;Rec: Numero (Integer)
(define interactions-system-norec   
  (lambda (lstCH user)
...
;Dom: Sistema x Opcion (String or Integer)
;Rec: Sistema
(define OpFinal-norec (lambda (CbNum FwNum Op lstCb)                
            ...
            (list (get-system-name system)
                  ...
                  (cons (list
                        ...
                        (current-seconds)
                                 (cons (cadr (reverse(last-interaction (get-system-CurrentUser system) (get-system-ChatHistory system)))) ;;CbNum
                                       (cons (car (reverse (last-interaction (get-system-CurrentUser system) (get-system-ChatHistory system)))) ;;FwNum
                                             (cons (OpFinal-norec                                                                                              ;; Funcion que permite un string como opcion
                                                   ...
                                                   (ch-fl-op
                                                          ...
                                                          (cadr (reverse (last-interaction (get-system-CurrentUser system)(get-system-ChatHistory system))))
                                                          (car  (reverse (last-interaction (get-system-CurrentUser system)(get-system-ChatHistory system))))
                                                          (OpFinal-norec                                                                                       ;; Funcion que permite un string como opcion
                                                                  (cadr (reverse(last-interaction (get-system-CurrentUser system) (get-system-ChatHistory system))))
                                                                  (car (reverse (last-interaction (get-system-CurrentUser system) (get-system-ChatHistory system))))
                                                                   op
                                                                  (get-system-chatbots system))
...
;Dom: Numero (Integer/id Chatbot) x list (Lista de chatbots)
;Rec: Lista (lista con el chatbot de interes)
(define ObCb                                   
  (lambda (cb-id lstCb)
     (filter (lambda (x) (equal? cb-id (car x))) lstCb)))

;Se obtiene el Flujo de interes (donde estan las opciones que se buscan)
;Dom: Numero (Integer/id Chatbot) x list (Lista de flujos)
;Rec: Lista (lista con el flujo de interes)
(define ObFw                                  
  (lambda (fw-id lstFw)
   (filter (lambda (x) (equal? fw-id (car x))) lstFw)))

;Se obtienen los nombres de las opciones disponibles.
;Dom: list (Lista de opciones)
;Rec: Lista (lista, donde cada elemeto contiene el id de la opcion y la descripcion de la opcion)
(define ObOpciones                                  
  (lambda (lstOp)
...
;Dom: Sistema x Usuario
;Rec: Lista (Con la informacion historica de interacciones del usuario en cuestion)
...
;Dom: Sistema x Numero de Chatbot x Numero de flujo
;Rec: Numero (Integer)
...
;Dom: Numero inf (Integer) x Numero sup (Integer)
;Rec: Numero
...
;Dom: Sistema x Acumulador (Integer) x Numero de interacciones a simular (Integer) x seed (Integer)
;Rec: Sistema
(define system-simulate-aux (lambda (system Acc Num_ite seed)                             
                              (if (< Acc Num_ite)                                     
                                     ...
                                     (system-synthesis (system-talk-simulator system (usuario_simulado seed)) (usuario_simulado seed)))
                              ))
                              

; Simulador propiamente tal
;Dom: Sistema x Numero de interacciones a simular (Integer) x seed (Integer)
;Rec: Dspliegue de interacciones
(define system-simulate (lambda (system Num_ite seed)
                 (system-simulate-aux system 1 Num_ite seed)))  
...
(define s18 (system-simulate s0 5 32131))       s18
