...
(define OpFinal    (lambda (Op lstOp)                    ; (OpFinal Op  (lstOpciones FwNum (lstFlujos CbNum lstCb ) ) )
                     (if (null? lstOp) null
                         (if  (or (equal? Op (car (car lstOp)))  (member Op (reverse(car lstOp))) )
                               (car (car lstOp))
...
(define OpFinal-sr (lambda (CbNum FwNum Op lstCb)                
                     (filter ( lambda (x)  (or (equal? Op (car (car x)))   (member x (car (reverse x))) ))
                             (car (car (reverse (filter (lambda (x) (equal? FwNum (car (car x)))) (car (reverse (filter (lambda (x) (equal? CbNum (car (car x)))) lstCb))) )))) )))
...
;------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;DEFINE SIMULADOR (#15)
; Cuenta las opciones asociadas a un cbNum fwNum
...
; Generar un usuario dado un codigo numerico
(define usuario_simulado (lambda (numero)  
          (string-append "user" (number->string numero)))) 

; Funcion que permite interactuar con el sistema con un usuario que no es parte de la lista de Users
...
(define system-simulate-aux (lambda (system Acc Num_ite seed)                             
                              (if (< Acc Num_ite)
                                 (begin
                                     ...
                                     (system-simulate-aux system (+ 1 Acc) Num_ite seed))
                                    (system-talk-simulator system (usuario_simulado seed))  )))
...
;(define system-simulate-aux (lambda (system Acc Num_ite seed funcion)                             
 ;                             (if (< Acc Num_ite)
  ;                                (begin
                                  ;(system-talk-simulator system (usuario_simulado seed)))
    ;                                (system-simulate-aux system (+ Acc 1) Num_ite seed))  ))
                              
      

;(define system-simulate (lambda (sistema num_iteraciones semilla)
  ;                        (if (< Acc num_iteraciones)
   ;                        (system-simulate-aux (sistema Acc seed)
...
(define s11 (system-talk-rec-sr s10 2))           s11
(define s12 (system-talk-rec-sr s11 1))           s12
(define s13 (system-talk-rec-sr s12 2))           s13
